...
namespace planner
{
    ...
    public:
            ...
            std::string lidar_topic)
        {
            ...
            obj_follow_linear = equation_generation.straightLineEquation(2, kMaxLinearVel*0.8, 1, 0);
            obj_follow_angular = equation_generation.straightLineEquation(0, 0, 16, kMaxAngularVel);
            coord_follow_angular = equation_generation.straightLineEquation(0.3, 0.5, 3.14, kMaxAngularVel);
        };
    ...
    {   
        ...
        if (PrevState == kArucoFollowing)
        {
            ...
            ROS_INFO_STREAM("Checking for similar coordinates...");
            ...
            while (it != obj_locations.end())
            {
                ...
                {
                    same_coord = true;
                    ROS_INFO_STREAM("Duplicate Object detected going back to GPS Coordinate...");
            ...
            {
                goal_count++;
                ROS_INFO_STREAM("Unique Object found! Saving to map and updating CSV");
                obj_locations.insert(std::pair<int, Coordinates>(obj_locations.size() + 1, curr_location));
    ...
    void SensorCallback::callSearchPattern(SearchPatternType search_pattern_type)
    {
        ...
        case kPatternFollowing:
            ...
            case 0:
                ...
                ROS_INFO("time now %d", time_offset);
                     
                temp_time = (ros::Time::now() + ros::Duration(time_offset)).toSec();
